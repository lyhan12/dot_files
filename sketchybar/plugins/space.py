...
all_spaces = json.loads(os.popen('yabai -m query --spaces').read())
all_spaces = [space["index"] for space in all_spaces]
...
empty_spaces = [index for index in all_spaces if not index in spaces]

args = ' '.join([f'--set space.{space} label="{to_formatted_icons(apps)}" label.drawing=on' for space, apps in spaces.items()])
args += ' ' + ' '.join([f'--set space.{space} label="" label.drawing=on' for space in empty_spaces])
default_args = "--set '/space\..*/' background.drawing=on --animate sin 0"

print(f'sketchybar -m {default_args} {args}')
